<template>
    <div class="UserComment">
        <ul>
            <li class="user_comment_data" v-for="(item) in songlistcomment" :key="item" :class="[$store.state.theme == 'night' ? 'user_comment_data_night':'']">
                <div class="comment_user">
                    <!--                        评论用户头像-->
<!--                    <img :src="require('../../../public/img/'+user_img)"  class="user_img">-->
                    <img :src="item.user.avatarUrl"  class="user_img">
                    <!--                 Single_review[index].user.nickname       评论用户信息-->
                    <div class="user_material">
                        <span class="user_name">{{ item.user.nickname }}</span>
                        <span class="response_time">{{ dateFormat(item.time) }}</span>
                    </div>
                    <!--                        评论获赞数-->
                    <div class="praised">
                        <svg v-if="!item.liked" @click="fabulous(item)" xmlns="http://www.w3.org/2000/svg" width="0.8rem" height="0.8rem" viewBox="0 0 15.765 16">
                        ...
                        <svg v-if="item.liked" @click="fabulous(item)" xmlns="http://www.w3.org/2000/svg" width="0.8rem" height="0.8rem" viewBox="0 0 14.762 15">
                            <path id="Icon_feather-thumbs-up" data-name="Icon feather-thumbs-up" d="M12,8.25v-3A2.25,2.25,0,0,0,9.75,3l-3,6.75V18h8.46a1.5,1.5,0,0,0,1.5-1.275l1.035-6.75a1.5,1.5,0,0,0-1.5-1.725ZM6.75,18H4.5A1.5,1.5,0,0,1,3,16.5V11.25a1.5,1.5,0,0,1,1.5-1.5H6.75" transform="translate(-3 -3)" fill="#5ab4f0"/>
                        </svg>
