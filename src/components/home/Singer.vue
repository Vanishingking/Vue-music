<template>
    <div class="Singer">
        <ul>
            <li v-for="(item) in SingerData" :key="item">
                <div class="Singer_box">
                    <img :src="item.singer_image" class="Singer_image">
                    <span>{{ item.singer_name }}</span>
                    <div class="gender">
                        <!--             性别-->
                        <!--                      女-->
                        <svg v-show="item.account_gender == 2" xmlns="http://www.w3.org/2000/svg" width="0.72rem" height="0.72rem" viewBox="0 0 14.621 14.622">
                        ...
                        <!--                      男-->
                        <svg v-show="item.account_gender == 1" xmlns="http://www.w3.org/2000/svg" width="0.72rem" height="0.72rem" viewBox="0 0 14.724 14.728">
...
<!--                    <div style="z-index: 2">-->
<!--                        <el-button type="primary" round size="mini">关注</el-button>-->
<!--                    </div>-->
                    <div v-if="item.singer_id != null" class="Singer_btn" @click="Singer_next(item.singer_id)"></div>
                    <div v-if="item.user_id != null" class="user_btn" @click="User_next(item.user_id)"></div>
                </div>
            </li>
        </ul>
    </div>
...
<script>
    export default {
        ...
        methods:{
            ...
            User_next(id){
                this.$store.commit('advance')
                this.$router.push( `/user/userdynamic/${id}`)
            }
        }
    }
</script>
